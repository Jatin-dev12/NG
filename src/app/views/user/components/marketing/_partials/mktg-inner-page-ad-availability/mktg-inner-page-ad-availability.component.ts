...
@Component({
    selector: 'app-mktg-inner-page-ad-availability',
    templateUrl: './mktg-inner-page-ad-availability.component.html',
    styleUrls: ['./mktg-inner-page-ad-availability.component.scss'],
    standalone: true,
    imports: [NgbPopover, ReactiveFormsModule, NgbInputDatepicker, OrderPipe]
})
export class MktgInnerPageAdAvailabilityComponent {
	...
	ngOnInit(): void {
		this.categories.subscribe(data => {
			if (data) {
				this.categoriesArray = data?.items?.filter((val: any) => val.moduleId === 'property').map((filteredObj: any) => filteredObj);
			}
		});
	}
