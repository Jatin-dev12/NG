...
@Component({
    selector: 'app-bill-form-owner',
    templateUrl: './bill-form-owner.component.html',
    styleUrls: ['./bill-form-owner.component.scss'],
    standalone: true,
...
export class BillFormOwnerComponent {
	...
	get ownerRefrence() {
		return this.catalog?.agreement?.userCatalogOrdersReferences.find(val => val.relation === AdditionalEnums.USER_CATALOG_ROLE_OWNER) ?? null;
	}

	get owner() {
		return this.catalog?.agreement?.manager ?? null;
	}
	...
	SelectProperty() {
		...
		modalRef.componentInstance.agreement_status = this.gs.is_tenant ? '' : '';
