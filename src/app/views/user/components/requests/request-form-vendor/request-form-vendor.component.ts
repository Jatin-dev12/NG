...
import { CatalogSelectDialogComponent, VendorSearchDialogComponent } from 'src/app/dialogs';
import { EnumRxKey, AdditionalEnums, Enums } from 'src/app/enums';
...
@Component({
    selector: 'app-request-form-vendor',
    templateUrl: './request-form-vendor.component.html',
    styleUrls: ['./request-form-vendor.component.scss'],
    standalone: true,
...
export class RequestFormVendorComponent {
	...
	formValidation() {
		this.form = this.gs.formBuilder.group({
			...
			receivers: new UntypedFormControl([], [Validators.required]), // 35 Sender id
	...
	selectProperty() {
		...
		modalRef.componentInstance.agreement_status = '';
	...
	selectVendors() {
		const modalRef = this.modalService.open(VendorSearchDialogComponent, {
			windowClass: 'center billspop',
			backdrop: 'static',
			keyboard: false,
			animation: true,
		...
		modalRef.componentInstance.vendor_ids = this.form.value?.receivers ?? [];
