...
@Component({
    selector: 'app-profile-two-step-verification-dialog',
    templateUrl: './profile-two-step-verification-dialog.component.html',
    styleUrls: ['./profile-two-step-verification-dialog.component.scss'],
    standalone: true,
    imports: [ReactiveFormsModule, FormSubmitDirective, ControlErrorsDirective, NgxIntlTelInputModule, LaddaDirective]
})
export class ProfileTwoStepVerificationDialogComponent {
	...
	ngOnInit(): void {
		...
		this.settingsObservable.subscribe(data => {
			...
			if (data && data?.items) {
				this.item = data?.items?.find((val: any) => val.key === 'auth2_verification');
